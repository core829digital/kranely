...
export const syncSupplierRole = mutation({
    ...
    handler: async (ctx) => {
        ...
        // Only upgrade from client role
        if (caller.role !== "client") return false;
...
export const finalizeKranelyQuote = mutation({
    args: {
        request_id: v.id("supplier_requests"),
    ...
    handler: async (ctx, args) => {
        ...
        // 1. Update the supplier request with final Kranely data
        await ctx.db.patch(args.request_id, {
            platform_margin_price: args.margin_price,
            platform_final_doc_id: args.final_doc_id,
            status: "accepted",
        });
