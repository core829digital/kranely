...
export default function InteractiveCalendar({ appointments, onCreateEvent, onUpdateEvent, onDeleteEvent }) {
  ...
  const handleDownloadICS = (appointment) => {
    ...
    const icsContent = [
      'BEGIN:VCALENDAR',
      'VERSION:2.0',
      'PRODID:-//Kranely//Appointment//IT',
      'BEGIN:VEVENT',
      `UID:${appointment._id}@kranely.app`,
      `DTSTAMP:${formatICSDate(new Date())}`,
      `DTSTART:${formatICSDate(startDate)}`,
      `DTEND:${formatICSDate(endDate)}`,
      `SUMMARY:Appuntamento Kranely - ${appointment.project_type}`,
            ...
            return (
              <motion.div
                ...
                className={`min-h-[100px] p-2 border-r border-b border-[#f8f9fa]/5 cursor-pointer transition-all ${!isCurrentMonth ? 'bg-[#343a40]/20' : 'bg-transparent hover:bg-[#495057]/30'
                  } ${isToday ? 'ring-2 ring-[#FFC703]' : ''} ${isSelected ? 'bg-[#495057]/50' : ''}`}
              >
                <div className={`text-sm font-medium mb-1 ${!isCurrentMonth ? 'text-[#6c757d]' : isToday ? 'text-[#FFC703]' : 'text-[#f8f9fa]'
                  }`}>
                  {format(date, 'd')}
                </div>
