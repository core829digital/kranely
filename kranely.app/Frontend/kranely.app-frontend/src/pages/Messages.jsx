...
export default function Messages() {
  ...
  // Auto-open conversation when navigated from Quotes with ?clientEmail=xxx
  ...
  const handleDeleteConversation = async () => {
    ...
    if (!confirm(`Are you sure you want to delete the conversation with ${selectedConversation.name}? This action is irreversible.`)) {
      return;
    }
  ...
  const emojis = ['ðŸ˜Š', 'ðŸ˜‚', 'â¤ï¸', 'ðŸ‘', '🎉', 'ðŸ”¥', 'âœ¨', 'ðŸ‘', 'ðŸ™Œ', 'ðŸ’¯'];
    ...
    <div className="min-h-screen relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #1C1A18 0%, #232323 60%, #2a2826 100%)' }}>
      ...
      <div className="pt-0 relative z-10 min-h-screen pb-safe">
        {convexUser === undefined ? (
          ...
          <div className="h-[calc(100vh-76px)] flex items-center justify-center text-white/25">User not found in database.</div>
        ) : (!isAdmin && !isClient && !isSupplier && !isWorker) ? (
          <div className="h-[calc(100vh-76px)] flex items-center justify-center text-center px-4">
            <div>
              <h2 className="text-xl text-white mb-2">Limited Access</h2>
              <p className="text-white/40">Book an appointment to unlock chat with the administration.</p>
              <Link to="/MyAppointments" className="mt-4 inline-block px-4 py-2 rounded-lg font-semibold text-[#1C1A18]" style={{ background: '#FFC703' }}>
                Book Appointment
              </Link>
            </div>
          </div>
          ...
          <>
            <div className="max-w-7xl mx-auto px-2 sm:px-3 lg:px-4 xl:px-6 py-2 sm:py-3 lg:py-4 xl:py-6">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-2 sm:gap-3 lg:gap-4 xl:gap-6" style={{ height: 'calc(100vh - 100px)' }}>
                {/* Conversations List */}
                <div className="lg:col-span-1 bg-[#1C1A18]/ backdrop-blur-xl rounded-xl lg:rounded-2xl border border-white/ overflow-hidden flex flex-col max-h-full">
                  <div className="p-3 sm:p-4 lg:p-6 border-b border-white/ flex flex-col gap-4 flex-shrink-0">
                    <div className="flex items-center justify-between">
                      <h2 className="text-base sm:text-lg lg:text-xl font-medium text-white">Conversations</h2>
                      <div className="flex gap-2">
                        <button
                          onClick={() => setVerifyModalOpen(true)}
                          className="p-2 rounded-lg hover:bg-white/ transition-all text-white/40 hover:text-white"
                    ...
                    {isAdmin && (
                      <div className="flex bg-black/20 p-1 rounded-lg border border-white/5">
                        <button
                          ...
                          className={`flex-1 py-1.5 text-xs sm:text-sm rounded-md transition-all ${conversationFilter === 'all' ? 'bg-[#FFC703]/20 text-[#FFC703] shadow-sm' : 'text-[#F0EBE8]/40 hover:text-[#F0EBE8]'}`}
                        >
                          All
                        </button>
                        <button
                          onClick={() => setConversationFilter('client')}
                          className={`flex-1 py-1.5 text-xs sm:text-sm rounded-md transition-all ${conversationFilter === 'client' ? 'bg-[#FFC703]/20 text-[#FFC703] shadow-sm' : 'text-[#F0EBE8]/40 hover:text-[#F0EBE8]'}`}
                        >
                          Clients
                        </button>
                        <button
                          onClick={() => setConversationFilter('supplier')}
                          className={`flex-1 py-1.5 text-xs sm:text-sm rounded-md transition-all ${conversationFilter === 'supplier' ? 'bg-[#FFC703]/20 text-[#FFC703] shadow-sm' : 'text-[#F0EBE8]/40 hover:text-[#F0EBE8]'}`}
                        >
                          Suppliers
                        </button>
                        <button
                          onClick={() => setConversationFilter('collaborator')}
                          className={`flex-1 py-1.5 text-xs sm:text-sm rounded-md transition-all ${conversationFilter === 'collaborator' ? 'bg-[#FFC703]/20 text-[#FFC703] shadow-sm' : 'text-[#F0EBE8]/40 hover:text-[#F0EBE8]'}`}
                        >
                          Staff
                        </button>
                      </div>
                    ...
                    {conversations.length === 0 ? (
                      ...
                      conversations.map((conv) => (
                        <motion.div
                          ...
                          className={`p-3 sm:p-4 border-b border-white/ cursor-pointer transition-colors ${selectedConversation?._id === conv._id
                            ? 'bg-white/'
                            : 'hover:bg-white/'
                            }`}
                        >
                          <div className="flex items-center gap-2 sm:gap-3">
                            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center font-bold text-sm sm:text-base flex-shrink-0 text-[#1C1A18]" style={{ background: 'linear-gradient(135deg, #FFC703 0%, #e6b200 100%)' }}>
                              {conv.name?.[0] || 'C'}
                            </div>
                            <div className="flex-1 min-w-0">
                              <div className="font-medium text-white truncate text-sm sm:text-base">
                                {!isAdmin && conv?.is_admin_chat ? 'Kranely' : (conv?.name || 'Conversation')}
                              </div>
                              <div className="text-xs sm:text-sm text-white/40 truncate">
                ...
                {/* Chat Area */}
                <div className="lg:col-span-2 bg-[#1C1A18]/ backdrop-blur-xl rounded-xl lg:rounded-2xl border border-white/ flex flex-col overflow-hidden max-h-full">
                  {!selectedConversation ? (
                    <div className="flex-1 flex items-center justify-center text-white/40 p-4">
                      <div className="text-center">
                        <MessageSquare size={40} className="mx-auto mb-2 sm:mb-4 opacity-50 sm:w-16 sm:h-16" />
                        <p className="text-sm sm:text-base lg:text-lg">Select a conversation</p>
                      </div>
                    </div>
                  ) : (
                    <>
                      {/* Chat Header */}
                      <div className="p-3 sm:p-4 lg:p-6 border-b border-white/ flex-shrink-0 flex justify-between items-center">
                        <h3 className="text-base sm:text-lg lg:text-xl font-medium text-white truncate">
                        ...
                        {messages.map((msg) => {
                          ...
                          if (isSystemMsg) {
                            return (
                              ...
                              >
                                <div className="bg-[#535252]/ border border-white/ rounded-full px-4 py-2 text-xs text-white/40">
                                  {msg.content}
                                </div>
                              </motion.div>
                            );
                          ...
                          return (
                            ...
                            >
                              <div className="w-8 h-8 rounded-full flex items-center justify-center text-[#1C1A18] font-bold text-sm flex-shrink-0" style={{ background: 'linear-gradient(135deg, #FFC703 0%, #e6b200 100%)' }}>
                                {msg.sender_email[0].toUpperCase()}
                              </div>
                              <div className={`flex flex-col ${isOwn ? 'items-end' : 'items-start'} max-w-[70%]`}>
                                <div className={`rounded-2xl px-4 py-2 ${isOwn
                                  ? 'text-[#1C1A18] font-medium'
                                  : 'bg-white/5 border border-white/10 text-[#F0EBE8]'
                                  } ${msg.is_ephemeral ? 'opacity-75 border border-yellow-500/30' : ''} relative group`}
                                  style={isOwn ? { background: 'linear-gradient(135deg, #FFC703 0%, #e6b200 100%)' } : {}}>

                                  {isAdmin && !isSystemMsg && (
                                    <button
                                      onClick={() => handleDeleteMessage(msg._id)}
                                      className="absolute -top-2 -right-2 p-1 bg-[#141210] rounded-full border border-white/10 text-red-400 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-900/20 z-10"
                                      ...
                                      {msg.message_type === 'file' && (
                                        <div className="mt-2 p-3 bg-black/20 rounded-xl border border-white/10 flex items-center justify-between gap-4 min-w-[200px] group/file">
                                          <div className="flex items-center gap-3 overflow-hidden">
                                            <div className="w-10 h-10 rounded-lg bg-[#FFC703]/20 flex items-center justify-center flex-shrink-0 border border-[#FFC703]/30">
                                              <FileText size={20} className="text-[#FFC703]" />
                                            </div>
                                            <div className="min-w-0">
                                              <p className="text-sm font-medium text-white truncate">{msg.file_name}</p>
                                              <p className="text-[10px] text-white/50 uppercase tracking-wider">PDF Document</p>
                                            </div>
                                          </div>
                                          <div className="flex gap-1">
                                            <Button
                                              size="icon"
                                              variant="ghost"
                                              className="h-8 w-8 rounded-full hover:bg-[#FFC703]/20 hover:text-[#FFC703] text-white/70"
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                  <button
                                    ...
                                    className={`flex items-center gap-1 text-xs ${hasLiked ? 'text-red-400' : 'text-white/40'
                                  ...
                                  {msg.is_ephemeral && (
                                    <span className="text-xs text-yellow-500" title="Ephemeral message - will disappear after 7 days">
                                      â±ï¸
                                    </span>
                                  )}
                                  <span className="text-[10px] text-white/40 ml-auto">
                                    {new Date(msg._creationTime).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}, {new Date(msg._creationTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                  </span>
                                </div>
                              </div>
                            </motion.div>
                      ...
                      <div className="p-2 sm:p-3 lg:p-4 border-t border-white/ flex-shrink-0">
                        ...
                        {isEphemeral && (
                          <div className="mb-2 px-2 sm:px-3 py-1.5 sm:py-2 bg-yellow-500/10 border border-yellow-500/30 rounded-lg text-xs text-yellow-500 flex items-center gap-2">
                            â±ï¸ Ephemeral message - will disappear after 7 days
                            <button
                        ...
                        <div className="flex items-end gap-2">
                          <div className="flex gap-1">
                            <Button
                              ...
                              className="text-white/70 hover:text-white hover:bg-white/"
                            >
                              <Plus size={20} />
                            </Button>
                            <Button
                            ...
                            <Button
                              ...
                              className={`${isEphemeral ? 'text-yellow-500' : 'text-white/70'} hover:text-white hover:bg-white/`}
                              title="Ephemeral message (7 days)"
                            >
                              <Clock size={20} />
                            </Button>
                          </div>
                          ...
                          <div className="flex-1 relative">
                            <Input
                              ...
                              className="bg-[#1C1A18] border-white/ text-white pr-12"
                            />
                            ...
                            <AnimatePresence>
                              {showEmojiPicker && (
                                <motion.div
                                  ...
                                  animate={{ opacity: 1, y: 0 }}
                                  exit={{ opacity: 0, y: 10 }}
                                  className="absolute bottom-full right-0 mb-2 bg-[#535252] border border-white/ rounded-xl p-3 grid grid-cols-5 gap-2 shadow-xl"
                                >
                                  {emojis.map((emoji) => (
                                    <button
                                      key={emoji}
                            ...
                            <AnimatePresence>
                              {showAttachMenu && (
                                <motion.div
                                  ...
                                  animate={{ opacity: 1, y: 0 }}
                                  exit={{ opacity: 0, y: 10 }}
                                  className="absolute bottom-full left-0 mb-2 bg-[#535252] border border-white/ rounded-xl overflow-hidden shadow-xl"
                                >
                                  ...
                                  <button
                                    ...
                                    className="flex items-center gap-3 px-4 py-3 text-white hover:bg-white/ w-full transition-colors"
                                  >
                                    <Paperclip size={18} />
                                    File
                                  </button>
                          ...
                          <Button
                            ...
                            className="font-bold text-[#1C1A18] disabled:opacity-40"
              ...
              {showContactsList && (
                ...
                >
                  <motion.div
                    ...
                    className="bg-[#1C1A18]/ backdrop-blur-xl border border-white/ rounded-3xl p-6 max-w-md w-full mx-4 max-h-[70vh] overflow-y-auto"
                  >
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-xl font-medium text-white">Select Contact</h3>
                      <button
                        onClick={() => setShowContactsList(false)}
                        className="p-2 rounded-lg hover:bg-white/ transition-all"
                      >
                    ...
                    <div className="relative mb-4">
                      ...
                      <Input
                        ...
                        className="pl-9 bg-[#141210] border-white/10 text-white placeholder:text-white/25"
                      />
                    </div>

                    {!isAdmin ? (
                      <Button
                        className="w-full mb-4 bg-[#FFC703] hover:bg-[#FFC703]/80 text-[#1C1A18] font-bold"
                        onClick={() => {
                      ...
                      <div className="flex flex-wrap gap-1 mb-4 bg-[#141210]/ p-1 rounded-xl">
                        {['All', 'Clients', 'Suppliers', 'Collaborators', 'Basic Users', 'Admin'].map((tab) => (
                          <Button
                            key={tab}
                            variant="ghost"
                            size="sm"
                            ...
                            className={`flex-1 text-[10px] sm:text-xs rounded-lg transition-all ${contactFilter === tab.toLowerCase().replace(' ', '_')
                              ? 'bg-[#FFC703]/20 text-white shadow-lg shadow-[#FFC703]/20'
                              : 'text-white/40 hover:text-white hover:bg-white/'
                              }`}
                          >
                            {tab}
                          </Button>
                      ...
                      {!isSupplier && allUsers
                        ...
                        .map((contact) => (
                          <button
                            ...
                            className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-[#535252]/ transition-all text-left"
                          >
              ...
              <DialogContent className="bg-gradient-to-br from-[#535252] to-[#535252] border-white/ text-white">
                <DialogHeader>
                  <DialogTitle className="text-white">
                    Upload {uploadType === 'camera' ? 'from Camera' : uploadType === 'image' ? 'Photo' : uploadType === 'video' ? 'Video' : 'File'}
                  </DialogTitle>
                </DialogHeader>
                <div className="mt-4">
                  <Input
                    ...
                    className="bg-[#1C1A18]/ border-white/ text-white file:bg-white/ file:text-white"
                  />
                </div>
              </DialogContent>
            </Dialog>
            ...
            <Dialog open={verifyModalOpen} onOpenChange={setVerifyModalOpen}>
              <DialogContent className="bg-[#1C1A18] border-white/ text-white">
                <DialogHeader>
                  <DialogTitle>Account Verification</DialogTitle>
                </DialogHeader>
                <div className="space-y-4 py-4">
<p className="text-sm text-white/40">Enter the access code to verify your account or activate business features.</p>
                  <Input
                  ...
                  <Button
                    ...
                    className="w-full bg-[#FFC703] hover:bg-[#FFC703]/80 text-[#1C1A18] font-bold"
                  >
                    Verify Code
                  </Button>
                </div>
...
function DocumentSelectorModal({ isOpen, onClose, onSelect }) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="bg-white/5 border-white/10 text-white max-w-md h-[80vh] flex flex-col">
        <DialogHeader>
<DialogTitle className="flex items-center gap-2">
        ...
        <div className="relative mb-4">
          ...
          <Input
            ...
            className="pl-9 bg-[#141210] border-white/10 text-white"
          />
        </div>

        <div className="flex-1 overflow-y-auto space-y-2 pr-2">
          {filteredDocs.length === 0 ? (
            <p className="text-center text-white/40 py-8">No documents found</p>
          ) : (
            filteredDocs.map(doc => (
              <button
                key={doc._id}
                onClick={() => onSelect(doc)}
                className="w-full text-left p-3 rounded-xl bg-[#535252]/ hover:bg-[#535252]/ border border-white/ hover:border-white/ transition-all group"
              >
                <div className="flex items-center justify-between mb-1">
                  <span className="font-medium text-sm text-white truncate">{doc.title}</span>
                  <span className="text-[10px] text-white/40 bg-[#141210] px-1.5 py-0.5 rounded uppercase">{doc.file_type?.split('/')[1] || 'FILE'}</span>
                </div>
