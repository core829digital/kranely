...
const plans = [
  { name: "Free", price: "0", period: "/mese", features: ["1 organizzazione", "Fino a 5 utenti", "Funzionalità base", "Supporto email"], cta: "Inizia Gratis" },
  { name: "Pro", price: "97", period: "/mese", features: ["Utenti illimitati", "White-label base", "Tutti i moduli", "Chat integrata", "Calcolatore edilizia", "Supporto prioritario"], cta: "Scegli Pro", highlighted: true },
  { name: "Enterprise", price: "Custom", period: "", features: ["White-label completo", "Custom domain", "API access", "SLA garantito", "Supporto dedicato", "Formazione inclusa"], cta: "Contattaci" },
]
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            {plans.map((plan) => (
              <div key={plan.name} className={`p-8 rounded-xl border ${plan.highlighted ? "border-kranely-accent/50 bg-kranely-accent/5" : "border-white/10 bg-white/[0.02]"}`}>
                ...
                <Link href="/sign-up"><Button className={`w-full ${plan.highlighted ? "bg-kranely-accent text-kranely-app-bg hover:bg-kranely-accent/90" : "bg-white/10 text-white hover:bg-white/20"}`}>{plan.cta}</Button></Link>
              </div>
            ))}
          </div>
        </div>
