    ...
    <div className="min-h-screen bg-kranely-app-bg">
      <nav className="fixed top-0 left-0 right-0 z-50 border-b border-white/10 bg-kranely-app-bg/80 backdrop-blur-xl">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 items-center justify-between">
            ...
            <Link href="/" className="text-sm text-white/60 hover:text-white transition-colors">Torna alla Home</Link>
          </div>
        </div>
      </nav>
      ...
      <div className="pt-24 pb-20">
        <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
          ...
          {posts === undefined ? (
            <div className="flex justify-center"><div className="w-8 h-8 border-2 border-kranely-accent border-t-transparent rounded-full animate-spin" /></div>
          ) : posts.length === 0 ? (
              ...
              {posts.map((post) => (
                <Link key={post._id} href={`/blog/${post.slug}`} className="group p-6 rounded-xl border border-white/10 bg-white/[0.02] hover:border-kranely-accent/30 transition-all">
                  <div className="flex items-center gap-2 text-xs text-white/40 mb-3">
                    ...
                    {post.publishedDate && <span className="flex items-center gap-1"><Calendar className="w-3 h-3" />{new Date(post.publishedDate).toLocaleDateString("it-IT")}</span>}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <ArrowRight className="w-4 h-4 text-kranely-accent opacity-0 group-hover:opacity-100 transition-opacity" />
                  </div>
                </Link>
              ))}
            </div>
