    ...
    return (
        <div className="min-h-screen bg-[#212529] flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
                <Card className="bg-[#343a40] border-[#495057] shadow-2xl">
                    <CardHeader className="text-center">
                        ...
                        <p className="text-sm text-[#adb5bd] mt-2">
                            Benvenuto nel team IWHome. Inserisci la password temporanea ricevuta su WhatsApp per attivare il tuo profilo.
                        </p>
                    </CardHeader>
                    <CardContent className="space-y-6">
                        <SignedIn>
                            {status === 'success' ? (
                                <div className="text-center py-4">
                                    ...
                                    <h3 className="text-xl text-[#f8f9fa] mb-2">Profilo Attivato!</h3>
                                    <p className="text-sm text-[#adb5bd] mb-6">Il tuo account è stato collegato correttamente. Verrai reindirizzato alla dashboard in pochi secondi...</p>
                                    <Button onClick={() => navigate('/Dashboard')} className="w-full bg-indigo-600 hover:bg-indigo-700">
                                ...
                                <>
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-xs text-[#adb5bd]">Password Temporanea (6 cifre)</label>
                                        <Input 
                                            type="text" 
                                            placeholder="Inserisci password..." 
                                            value={password}
                                            onChange={e => setPassword(e.target.value)}
                                            className="bg-[#495057] border-[#6c757d] text-[#f8f9fa] text-center text-2xl tracking-[0.5em] h-14"
                                            maxLength={6}
                                        />
                                    </div>
                        ...
                        <SignedOut>
                            <div className="text-center py-4 space-y-4">
                                <div className="flex items-start gap-2 bg-indigo-500/10 border border-indigo-500/20 rounded-lg p-3 text-left">
                                    <AlertCircle size={16} className="text-indigo-400 flex-shrink-0 mt-0.5" />
                                    <p className="text-xs text-indigo-300">Devi creare un account o accedere per poter collegare il tuo profilo staff.</p>
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <SignInButton mode="modal" forceRedirectUrl={window.location.href}>
                                        <Button variant="outline" className="w-full border-indigo-500/50 text-indigo-400 hover:bg-indigo-500/10">Accedi</Button>
                                    </SignInButton>
                                    <SignUpButton mode="modal" forceRedirectUrl={window.location.href}>
                                        <Button className="w-full bg-indigo-600 hover:bg-indigo-700 text-white">Registrati</Button>
                                    </SignUpButton>
                                </div>
                            </div>
                        </SignedOut>
